...
class SalesTrend(QWidget):
    ...
    def paintEvent(self, event):
        ...
        points = [
            (
                int(rect.left() + i * step),
                int(rect.bottom() - value / maximum * rect.height()),
            )
...
class InvoiceDetailsDialog(QDialog):
    def __init__(self, session_factory, invoice_id, parent=None):
        ...
        with session_factory() as session:
            ...
            customer = (
                session.get(Customer, invoice.customer_id)
                if invoice.customer_id
                else None
            )
            cashier = session.get(User, invoice.cashier_id)
            cashier_name = cashier.display_name if cashier else "Unknown cashier"
            items = list(invoice.items)
        title = QLabel(invoice.local_invoice_number)
        title.setStyleSheet("font-size:22px;font-weight:800")
        layout.addWidget(title)
        layout.addWidget(
            QLabel(
                f"Date: {invoice.created_at.strftime('%Y-%m-%d %H:%M')}   |   Customer: {customer.name if customer else 'Walk-in Customer'}   |   Cashier: {cashier_name}\nPayment: {(invoice.payment_method or 'unknown').title()}   |   Status: {(invoice.status or 'unknown').title()}"
            )
        )
        table = QTableWidget(len(items), 4)
        table.setHorizontalHeaderLabels(["Item", "Qty", "Unit Price", "Amount"])
        ...
        total = QLabel(
            f"Subtotal: {money(invoice.subtotal_cents)}   Discount: {money(invoice.discount_cents)}   Tax: {money(invoice.tax_cents)}\nTotal: {money(invoice.total_cents)}"
        )
        total.setAlignment(Qt.AlignmentFlag.AlignRight)
        total.setStyleSheet("font-size:18px;font-weight:800")
...
class SalesPage(QWidget):
    def __init__(
        self, session_factory, user: User, open_pos_callback=None, parent=None
    ):
        super().__init__(parent)
        self.session_factory = session_factory
        self.user = user
        self.open_pos_callback = open_pos_callback
        with session_factory() as session:
        ...
        root.setSpacing(14)
        heading = QHBoxLayout()
        sales_title = QLabel("Sales"); sales_title.setObjectName("title"); heading.addWidget(sales_title)
        sales_subtitle = QLabel("Track invoices, payments and transaction history")
        sales_subtitle.setObjectName("muted")
        heading.addWidget(sales_subtitle)
        heading.addStretch()
        ...
        self.method.addItems(["All Payment Methods", "Cash", "Card", "Credit"])
        self.method.currentIndexChanged.connect(self.refresh)
        self.cashier = QComboBox()
        self.cashier.currentIndexChanged.connect(self.refresh)
        ...
        self.trend = SalesTrend()
        trend_box.addWidget(self.trend)
        right.addWidget(trend_panel)
        payment_panel = QFrame(); payment_panel.setObjectName("panel")
        self.payment_box = QVBoxLayout(payment_panel)
        self.payment_box.addWidget(QLabel("Payment Method Breakdown"))
        right.addWidget(payment_panel)
        best_panel = QFrame(); best_panel.setObjectName("panel")
        self.best_label = QLabel()
        self.best_label.setStyleSheet("font-size: 15px; font-weight: 700; color: #1671f8;")
        self.best_label.setAlignment(Qt.AlignmentFlag.AlignTop | Qt.AlignmentFlag.AlignLeft)
        best_box = QVBoxLayout(best_panel)
        best_box.addWidget(QLabel("Best Sales Day"))
    ...
    def card(self, title, value, color, pale_color="#f8fafc", note=""):
        ...
        icon_box.setStyleSheet(f"background: {pale_color}; border-radius: 12px;")
        icon_layout = QVBoxLayout(icon_box)
        icon_layout.setContentsMargins(0, 0, 0, 0)
        icon = QLabel("●")
        icon.setStyleSheet(f"color:{color};font-size:24px")
        ...
        text.addWidget(val)
        if note:
            note_label = QLabel(note); note_label.setObjectName("muted"); text.addWidget(note_label)
    ...
    def reload_cashiers(self):
        with self.session_factory() as session:
            users = session.execute(
                select(User.id, User.display_name).order_by(User.display_name)
            ).all()
        self.cashier.blockSignals(True)
        self.cashier.addItem("All Cashiers", None)
        for user_id, name in users:
            self.cashier.addItem(name, user_id)
        self.cashier.blockSignals(False)

    def query(self, session):
        query = (
            select(Invoice, Customer.name, User.display_name, func.count(SaleItem.id))
            .outerjoin(Customer, Customer.id == Invoice.customer_id)
            .join(User, User.id == Invoice.cashier_id)
            .outerjoin(SaleItem, SaleItem.invoice_id == Invoice.id)
            .group_by(Invoice.id)
        )
        ...
        if term:
            query = query.where(
                or_(
                    Invoice.local_invoice_number.ilike(f"%{term}%"),
                    Customer.name.ilike(f"%{term}%"),
                    SaleItem.product_name.ilike(f"%{term}%"),
                )
            )
        from_dt = datetime.combine(self.from_date.date().toPyDate(), time.min).replace(tzinfo=timezone.utc)
        to_dt = datetime.combine(self.to_date.date().toPyDate(), time.max).replace(tzinfo=timezone.utc)
        query = query.where(
        ...
        if self.method.currentIndex() > 0:
            query = query.where(
                Invoice.payment_method == self.method.currentText().lower()
            )
        if self.cashier.currentData():
            query = query.where(Invoice.cashier_id == self.cashier.currentData())
        if self.status.currentIndex() > 0:
            query = query.where(Invoice.status == self.status.currentText().lower())
    ...
    def refresh(self):
        ...
        with self.session_factory() as session:
            ...
            payment_rows = session.execute(
                ...
                .join(Invoice)
                .where(~Invoice.status.in_(["cancelled", "refunded"]))
        ...
        revenue = sum(
            i.total_cents
            for i in all_invoices
            if i.status not in {"cancelled", "refunded"}
        )
        paid = sum(i.status == "paid" for i in all_invoices)
        pending = sum(i.status == "pending" for i in all_invoices)
        ...
        metrics = [
            ...
            (
                "Total Invoices",
                str(len(all_invoices)),
                "#10b981",
                "#ecfdf5",
                "All transactions",
            ),
            ("Paid Invoices", str(paid), "#8b5cf6", "#f5f3ff", "Completed"),
            ("Pending Payments", str(pending), "#f59e0b", "#fffbeb", "Credit invoices"),
            ("Total Revenue", money(revenue), "#10b981", "#ecfdf5", "Net active sales"),
        ]
        for column, metric in enumerate(metrics):
            self.metrics.addWidget(self.card(*metric), 0, column)
        self.table.setRowCount(len(rows))
        for row_number, (invoice, customer_name, cashier_name, item_count) in enumerate(
            rows
        ):
            values = [
                invoice.local_invoice_number,
                invoice.created_at.strftime("%d %b %Y %H:%M"),
                customer_name or "Walk-in Customer",
                cashier_name,
                (invoice.payment_method or "Unknown").title(),
                item_count,
                money(invoice.total_cents),
                (invoice.status or "Unknown").title(),
            ]
            for column, value in enumerate(values):
                item = QTableWidgetItem(str(value))
                item.setData(
                    Qt.ItemDataRole.UserRole, invoice.id if column == 0 else None
                )
                if column == 7:
                    if value == "Paid":
                    ...
                    elif value in ("Cancelled", "Refunded"):
            ...
            for text, fn in (
                ("View", self.view_invoice),
                ("Print", self.print_invoice),
                ("Cancel", self.cancel_invoice),
                ...
                if required:
                    button.setEnabled(
                        required in self.permissions
                        and invoice.status not in {"cancelled", "refunded"}
                    )
                button.clicked.connect(
                    lambda _checked=False, iid=invoice.id, callback=fn: callback(iid)
                )
        ...
        for invoice in all_invoices:
            if invoice.status not in {"cancelled", "refunded"}:
                daily[invoice.created_at.date()] += invoice.total_cents
        days = [today - timedelta(days=offset) for offset in reversed(range(7))]
        self.trend.set_values([daily[day] for day in days])
        ...
        for method, amount in payment_rows:
            row = QHBoxLayout()
            row.addWidget(QLabel(method.title()))
            row.addStretch()
            row.addWidget(QLabel(money(amount or 0)))
            self.payment_box.addLayout(row)
        if daily:
            best_day, amount = max(daily.items(), key=lambda pair: pair[1])
            count = sum(i.created_at.date() == best_day for i in all_invoices)
            self.best_label.setText(
                f"{best_day.strftime('%A, %d %b %Y')}\n{money(amount)}\n{count} invoices"
            )
        else:
            self.best_label.setText("No sales yet")

    def new_sale(self):
        if self.open_pos_callback:
            self.open_pos_callback()

    def view_invoice(self, invoice_id):
        InvoiceDetailsDialog(self.session_factory, invoice_id, self).exec()

    def print_invoice(self, invoice_id):
        ReceiptDialog(self.session_factory, invoice_id, self).exec()
    ...
    def reverse(self, invoice_id, action):
        ...
        except (CheckoutError, PermissionDenied, ValueError) as exc:
            QMessageBox.warning(self, "Cannot update invoice", str(exc))
            return
        self.refresh()

    def cancel_invoice(self, invoice_id):
        self.reverse(invoice_id, "cancelled")

    def refund_invoice(self, invoice_id):
        self.reverse(invoice_id, "refunded")

    def export_csv(self):
        path, _ = QFileDialog.getSaveFileName(
            self, "Export Sales", "sales.csv", "CSV (*.csv)"
        )
        if not path:
            return
        with self.session_factory() as session:
        ...
        with open(path, "w", newline="", encoding="utf-8-sig") as file:
            ...
            for invoice, customer, cashier, count in rows:
                writer.writerow(
                    [
                        invoice.local_invoice_number,
    ...
    def print_report(self):
        with self.session_factory() as session:
            rows = session.execute(self.query(session)).all()
        html = (
            "<h1>OilMart Sales Report</h1><table border='1' cellspacing='0' cellpadding='5'><tr><th>Invoice</th><th>Date</th><th>Customer</th><th>Total</th><th>Status</th></tr>"
            + "".join(
                f"<tr><td>{i.local_invoice_number}</td><td>{i.created_at:%Y-%m-%d %H:%M}</td><td>{c or 'Walk-in'}</td><td>{money(i.total_cents)}</td><td>{i.status.title()}</td></tr>"
                for i, c, _, _ in rows
            )
            + "</table>"
        )
